...
const
  ...
  NODE_EVENT_LCC_LOGIN       = 1;
  NODE_EVENT_LCC_LOGOUT      = 2;
  NODE_EVENT_ON_INITIALIZED  = 3;
  NODE_EVENT_ON_PERMITTED    = 4;

type
  TLccCoreNode = class;

  TOnNodeEvent = procedure(Sender: TLccCoreNode; EventCode: Integer) of object;
  ...
  protected
    ...
    procedure DoNodeEvent(EventCode: Integer);
  ...
  public
    ...
    property OnEvent: TOnNodeEvent read FOnEvent write FOnEvent;
...
procedure TLccCoreNode.DoNodeEvent(EventCode: Integer);
begin
  if Assigned(OnEvent) then
    OnEvent(Self, EventCode);
end;
...
begin
 // LoginTimer.Enabled := False;   // This locks up on some systems (Raspberry Pi)
  ...
  begin
    ...
    SendEvents;
    FLoggedIn := True;
    DoNodeEvent(NODE_EVENT_LCC_LOGIN);
  end;
end;
...
begin
  ...
  DoRequestSendMessage(WorkerMessage, False);
  FPermitted := True;
  DoNodeEvent(NODE_EVENT_ON_PERMITTED);
  WorkerMessage.LoadInitializationComplete(NodeID, AliasID);
  DoRequestSendMessage(WorkerMessage, False);
  FInitialized := True;
  DoNodeEvent(NODE_EVENT_ON_INITIALIZED);
end;
...
begin
  ...
  begin
    ...
    DoRequestSendMessage(WorkerMessage, False);
    DoNodeEvent(NODE_EVENT_LCC_LOGOUT);
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      FSeedNodeID[0] := TempNodeID[0];
      FSeedNodeID[1] := TempNodeID[1];
    end;
